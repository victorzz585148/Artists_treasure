...
if ($categorySelect === 'artwork') {
    $stmt = $mysqli->prepare("SELECT AK_ID as id, AK_NAME as name FROM `artwork` WHERE AK_NAME LIKE ?");
} elseif ($categorySelect === 'collection') {
    $stmt = $mysqli->prepare("SELECT COL_ID as id, COL_NAME as name FROM `collection` WHERE COL_NAME LIKE ?");
} else {
