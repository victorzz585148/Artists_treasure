$(document).ready(function() {
    ...
    // 展覽表單 - 選擇作品時添加至畫面
    $('#contestArtworks').on('select2:select', function(e) {
        ...
        $('#selectedArtworksContainer').append(`
            <div class="artwork-item" id="artwork-${selectedId}" style="margin-bottom: 10px;">
                ...
                <br>
                <input type="text" placeholder="輸入比賽作品別名（可選）" name="alias[${selectedId}]" style="margin-top: 5px;" autocomplete="off">
                <label for="contestrank">獲獎名次：</label>
                <input type="number" placeholder="輸入比賽作品名次（可選）"id="contestrank" name="contestrank" class = 'form-control form_input' autocomplete="off" required>
                <label for="contestamount">獲獎金額：</label>
                <input type="number" placeholder="輸入比賽作品獲獎金額（可選）"id="contestamount" name="contestamount" class = 'form-control form_input'autocomplete="off" required>
            </div>
        `);
    });
    ...
    window.removeArtwork = function(artworkId) {
        ...
        let selectedItems = $('#contestArtworks').val();
        selectedItems = selectedItems.filter(item => item !== artworkId);
        $('#contestArtworks').val(selectedItems).trigger('change');
    };

    // 當取消選取時，移除相應的元素
    $('#contestArtworks').on('select2:unselect', function(e) {
    ...
    $('#en-btn').on('click', function(event) {
        ...
        const requiredFields = [
            { selector: '#contestName', label: '賽事名稱' },
            { selector: '#contestLocation', label: '賽事地點' },
            { selector: '#contestOrganizer', label: '主辦單位' },
            { selector: '#contestStart', label: '開始時間' },
            { selector: '#contestEnd', label: '結束時間' },
        ];

        // 定義所有需要顯示的欄位（包括非必填）
        const allFields = [
            { selector: '#contestName', label: '展覽名稱' },
            { selector: '#contestLocation', label: '地點' },
            { selector: '#contestOrganizer', label: '主辦單位' },
            { selector: '#contestStart', label: '開始時間' },
            { selector: '#contestEnd', label: '結束時間' },
            { selector: '#contestInterduce', label: '賽事介紹' },
            { selector: '#contestRemark', label: '備註' }
        ];
